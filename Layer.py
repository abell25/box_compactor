class Layer:
  ...
  def packPresentInSpaces(self, present, closedSpaces):
    ...
    for shelf in closedSpaces:
      [used_shelf, packedPresent, closedShelf] = self.packPresentInClosed(shelf, present)
      if used_shelf:
        used_closed_shelf = True
        fits.append([packedPresent, closedShelf, shelf])
      
    if used_closed_shelf:
      # best fit -> shelf - present = sorted by unused space.
      #fits.sort(key=lambda x: x[1][0]*x[1][1] - x[0][1]*x[0][2])
      #packedPresent, closedShelf, shelf = fits[0] #best fit

      # worst fit -> shelf - present = sorted by leftover space.
      fits.sort(key=lambda x: x[1][0]*x[1][1] - x[0][1]*x[0][2], reverse=True)
      packedPresent, closedShelf, shelf = fits[0] #worst fit
